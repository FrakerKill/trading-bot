    ...
    while (true) {
        try {
            ...
            if (mPrice) {
                ...
                log(`Minimum price order: ${startPrice * (100 - (parseFloat(BS_PERCENT) * (initialBalance2 / parseFloat(BUY_ORDER_AMOUNT)))) / 100 }`)
                log(`Minimum Percent: ${100 - (parseFloat(BS_PERCENT) * (initialBalance2 / parseFloat(BUY_ORDER_AMOUNT))) / 100 }`)
                ...
                if (marketPrice < startPrice) {
                    ...
                    if (percent >= (parseFloat(BS_PERCENT) * (1 + (parseFloat(INCREMENTAL_VOLATILITY) * orders.length))))
...
const getBalances = async () => {
    ...
    assets.forEach(asset => {
        parsedBalances[asset] = parseFloat(_balances.find(coin => coin.asset === asset).free)
    })
...
async function init() {
    ...
    if (process.argv[5] !== 'resume') {
        ...
        store.put('fees', 0)
        const balances = await getBalances()
        if (ENTRY_PRICE_MANUAL) {
            store.put('entry_price', parseFloat(ENTRY_PRICE_MANUAL))
        } else {
